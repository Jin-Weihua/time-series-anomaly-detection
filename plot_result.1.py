...
data_prd1 = pd.read_csv(
    'result/{}/{}-prd4-40.csv'.format(model_name,model_name),
    sep=',',
    index_col=0,
    encoding='utf-8',
    parse_dates=True,
...
data_raw = data_raw1.loc[:,column].iloc[0:80000]
data_prd = data_prd1.loc[:,column].iloc[0:80000]

data = []
print(type(data_raw.index))
column = 'INZ15_ABCR2输入电流' 
...
plotly.offline.plot(data, filename='result/{}/train.html'.format(model_name), auto_open=True)

...
data_raw = pd.read_csv(
    '/Users/jinweihua/Downloads/dd.csv',
    sep=',',
    index_col=0,
    encoding='utf-8',
    parse_dates=True,
...
data_prd1 = pd.read_csv(
    'result/autoencoder7/autoencoder7-ano7-249-2.csv',
    sep=',',
    index_col=0,
    encoding='utf-8',
    parse_dates=True,
...
columns = ['INA2_A电池组放电电流','VNZ2MEA电压(S3R)']
column_norm = ['norm']

ylabel = 'Normalized value'

column1 = 'INA2_A电池组放电电流'
column2 = 'VNZ2MEA电压(S3R)'
...
ax.plot(data_raw[1500:4500].loc[:, column2], '-', color='green', linewidth=1.0,label='MEA voltage')
ax.plot(data_raw1[1500:4500].loc[:, column2], '-', color='C7', linewidth=1.0,label='MEA voltage(A)')
...
ax1 = fig1.add_subplot(1, 3, 2, sharey=ax)
ax1.spines['top'].set_visible(False)  #去掉上边框
ax1.spines['right'].set_visible(False) #去掉右边框
ax1.spines['left'].set_visible(False) #去掉左边框
ax1.plot(data_raw[1500:4500].loc[:, column1], '-', color='green', linewidth=1.0,label='Battery set discharge current')
ax1.plot(data_raw1[1500:4500].loc[:, column1], '-', color='C7', linewidth=1.0,label='Battery set discharge current(A)')
...
ax3 = fig1.add_subplot(1, 3, 3, sharey=ax)
...
# plt.show()
plt.savefig('result/10.eps',format='eps')
plt.savefig('result/10.tiff',format='tiff')

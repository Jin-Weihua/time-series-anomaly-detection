...
index = data_basic.index
dist = np.linalg.norm(dataset_basic - dataset_rolling, axis=-1).reshape(-1,1)
data_dist = pd.DataFrame(dist, index=index, columns=['norm']).plot.scatter()

clf = KMeans(n_clusters=2)
data_dist['label'] = clf.fit_predict(data_dist[['norm']])
column = ['INZ14_ABCR1输入电流']
data_basic1 = data_basic.loc[:,column].iloc[0:10000]
data_dist1 = data_dist.loc[:,['label']].iloc[0:10000]
data = []
data.append(go.Scatter(x=data_dist1.index, y=data_dist1['label'], mode='markers', name='label'))
column = 'INZ14_ABCR1输入电流'
data.append(go.Scatter(x=data_basic1.index, y=data_basic1[column], mode='markers', name=column))
data_dist2 = data_dist.iloc[0:10000]
# plotly.offline.plot(data, filename='w.html', auto_open=True)
...
for i in range(numSamples):
    ...
    clf.labels_[i]
    mark[clf.labels_[i]]
    plt.plot(dist[i][0], mark[clf.labels_[i]]) #mark[markIndex])
...
data_SAE = pd.read_csv(
    ...
    parse_dates=True,
    date_parser=dateparser)
dataset_SAE = data_SAE.as_matrix()
...
data_DAE = pd.read_csv(
    ...
    parse_dates=True,
    date_parser=dateparser)
dataset_DAE = data_DAE.as_matrix()
...
data_DAES = pd.read_csv(
    ...
    parse_dates=True,
    date_parser=dateparser)
dataset_DAES = data_DAES.as_matrix()
